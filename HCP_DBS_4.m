...
%Usage: run HCP_DBS_4.m

%The following steps are performed:

%1. Load in per-HCP-subject/per-ESTEL-patient-VTA structural connectivity matrices (*.csv files), computed in HCP_DBS_3.sh
%2. Retain columns/rows of interest (i.e., left and right VTAs to all other brain regions within the 429-region parcellation)
%3. Multiply each matrix element by subject-specific fibre-density proportionality coefficient (mu), output from SIFT2 (see HCP_DBS_3.sh)
%4. Average across HCP subjects (yielding per-ESTEL-patient connectivity values, for left and right VTAs separately)
%5. Sum together HCP subject-averaged matrices corresponding to left and right VTAs (yielding per-ESTEL-patient bilateral connectivity values, for left and right VTAs combined)
...
topdir = '/data/HCP-PP/MATLAB';
subject_list = load('/data/HCP_SUBJECTS.txt');
conndir = '/data/HCP-PP/VTA_CONNECTOMES';

addpath('/data/SOFTWARE'); %load in dependencies

allhcp_100subj_19vta_wdata_left_norm = zeros(19,429,100); %empty matrix to save mu-weighted data for LEFT vtas. Dimensions = 19 ESTEL patients x 429 regions x 100 HCP subjects
allhcp_100subj_19vta_wdata_right_norm = zeros(19,429,100); %empty matrix to save mu-weighted data for RIGHT vtas. Dimensions = 19 ESTEL patients x 429 regions x 100 HCP subjects
...
for s = 1:length(subject_list); %for each subject 
    ...
    for v = 1:19; %for each vta
         ...
         allhcp_100subj_19vta_wdata_left_norm(v,:,s) = wdata_norm(:,1)';
         allhcp_100subj_19vta_wdata_right_norm(v,:,s) = wdata_norm(:,2)'; 
         
    end
    
end

%average across 100 hcp subjects (retaining per-vta values)
allhcp_subjavg_19vta_wdata_left_norm=mean(allhcp_100subj_19vta_wdata_left_norm,3); %3rd dimension = 100 hcp subjects
allhcp_subjavg_19vta_wdata_right_norm=mean(allhcp_100subj_19vta_wdata_right_norm,3); %3rd dimension = 100 hcp subjects

%sum together left and right vta hcp-subject-averaged matrices
allhcp_subjavg_19vta_wdata_bilatsum_norm=allhcp_subjavg_19vta_wdata_left_norm + allhcp_subjavg_19vta_wdata_right_norm;

%average across 19 vtas (yielding a single 429-length vector - one value per region)
allhcp_subjavg_vtaavg_wdata_bilatsum_norm=mean(allhcp_subjavg_19vta_wdata_bilatsum_norm);

%for display purposes, square-root normalise the hcp-subject-averaged & vta-averaged vector - to "compress" the small number of high values and "spread out" the
%larger number of low values
allhcp_subjavg_vtaavg_wdata_bilatsum_norm_sqrt=sqrt(allhcp_subjavg_vtaavg_wdata_bilatsum_norm);

%fill brain atlas with hcp-subject-averaged, vta-averaged, and sqrt-normalised values 
atlas = nii_tool('load', fullfile(topdir, 'MNI6THGEN_1MM_HCPMMP1_TIAN_CERES_AAN_1-429.nii')); %load in atlas
allhcp_subjavg_vtaavg_vol=zeros(size(atlas.img)); %blank matrix equal size to atlas, for filling with results

%fill with results
for iroi= 1 : 429;    
        allhcp_subjavg_vtaavg_vol(find(atlas.img==iroi))=allhcp_subjavg_vtaavg_wdata_bilatsum_norm_sqrt(iroi);
end

%save as nii
atlas_filled1=atlas;
atlas_filled1.img=alllgs_subjavg_vtaavg_vol;
nii_tool('save', atlas_filled1, 'allhcp_subjavg_vtaavg_wdata_bilatsum_norm_sqrt.nii');

%okay, now back to per-bilateral-vta, hcp-subject-averaged data. first exclude vtas for patients with missing eeg data (ESTEL patients 9 & 18)
allhcp_subjavg_19vta_wdata_bilatsum_norm_exc=allhcp_subjavg_19vta_wdata_bilatsum_norm;
allhcp_subjavg_19vta_wdata_bilatsum_norm_exc([9,18],:)=[];

%find mean and sqrt of above data (for later use re: connectivity to areas of a priori eeg-fmri activation)
allhcp_subjavg_vtaag_wdata_bilatsum_norm_exc=mean(allhcp_subjavg_19vta_wdata_bilatsum_norm_exc);
allhcp_subjavg_vtaag_wdata_bilatsum_norm_exc_sqrt=sqrt(allhcp_subjavg_vtaag_wdata_bilatsum_norm_exc);
...
%correlate per-subject clinical outcomes with per-bilateral-vta hcp-subject-averaged connectivity strength
%DIARY OUTCOME !!!!
diary_conn_clin_corr=zeros(1,429);
for iroi=1:429;   
    conn=allhcp_subjavg_19vta_wdata_bilatsum_norm(:,iroi);
...
%correlate per-subj clinical outcomes with per-bilateral-vta hcp-subject-averaged connectivity strength
%EEG OUTCOME !!!!
eeg_conn_clin_corr=zeros(1,429);
for iroi=1:429;    
    conn=allhcp_subjavg_19vta_wdata_bilatsum_norm_exc(:,iroi); %note use of the *exc vector - patients with no eeg data excluded 
...
allhcp_subjavg_19vta_exc_eegdiaryavg_pfakeep=allhcp_subjavg_19vta_wdata_bilatsum_norm_exc(:,find(rpos_pfakeep_eegdiaryavg==1));

%sqrt-ed subject-average connectivity at each of the "kept" columns (for display purposes)
eediaryavg_rpos_pfakeep_connsubjavg_sqrt=rpos_pfakeep_eegdiaryavg.*allhcp_subjavg_vtaag_wdata_bilatsum_norm_exc_sqrt;
...
nii_tool('save', atlas_filled5, 'allhcp_subjavg_vtaavg_wdata_bilatsum_norm_sqrt_17subj_eegdiaryavgend3mths_GPFAMASKED.nii');

%sum connectivity across columns (i.e., for each ESTEL patient, total
%connectivity to the gpfa-retained parcels)
eegdiaryavg_connsum_rpos_pfakeep=sum(allhcp_subjavg_19vta_exc_eegdiaryavg_pfakeep,2);
